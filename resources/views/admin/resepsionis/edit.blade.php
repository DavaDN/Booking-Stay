        ...
        <div class="card-body">
            <form action="{{ route('resepsionis.update', $resepsionis->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control" value="{{ old('name', $resepsionis->name) }}" required>
                    @error('name') <small class="text-danger">{{ $message }}</small> @enderror
                </div>
                <div class="mb-3">
                    <label class="form-label">Email</label>
                    <input type="email" name="email" class="form-control" value="{{ old('email', $resepsionis->email) }}" required>
