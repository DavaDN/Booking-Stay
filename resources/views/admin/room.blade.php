...
<style>
    ...
    .search-bar button {
        ...
        padding: 6px 12px; border-radius: 0 8px 8px 0; cursor: pointer;
    }
    .room-card {
        border-radius: 10px; box-shadow: 0 3px 6px rgba(0,0,0,0.1);
        background: #fff; padding: 12px; font-size: 13px; margin-bottom: 20px;
    }
    .room-card .room-header { display: flex; justify-content: space-between; font-weight: bold; }
    .badge-status { font-size: 11px; padding: 3px 6px; border-radius: 6px; text-transform: capitalize; }
    .badge-available { background: #eafaf1; color: #27ae60; }
    .badge-occupied { background: #f0f4ff; color: #2980b9; }
    .badge-maintenance { background: #fff4f4; color: #e74c3c; }
    .room-footer { display: flex; justify-content: flex-end; gap: 8px; margin-top: 8px; }
    .room-footer button { border: none; background: none; cursor: pointer; }
    .room-footer .btn-edit { color: #2980b9; }
    .room-footer .btn-delete { color: #e74c3c; }
</style>
                ...
                <div class="room-card">
                    <div class="room-header">
                        ...
                        <span class="badge-status 
                            ...
                            {{ $room->status == 'booked' ? 'badge-occupied' : '' }}
                            {{ $room->status == 'maintenance' ? 'badge-maintenance' : '' }}">
                            {{ $room->status }}
                        </span>
                    </div>
...
<script>
    ...
    window.onclick = (e) => { if (e.target == modalAdd) closeAddModal(); }
